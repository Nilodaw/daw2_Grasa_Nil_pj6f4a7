...
function parse_order_line(string $ln): ?array
{
    ...
    $items = [];
    foreach (array_filter(explode(',', $p[5])) as $ch) {
        [$n, $q] = array_pad(explode('x', $ch, 2), 2, '0');
        $items[] = [
            'name' => $n,
            'qty'  => (int) $q,
        ];
...
function find_order_by_code(string $db, string $code): ?array
{
    if (!is_file($db)) {
        return null;
    }
